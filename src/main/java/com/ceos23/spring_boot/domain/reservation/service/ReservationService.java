...
public class ReservationService {
    ...
    public PaymentDataInfo requestInstantPayment(String paymentId, String email, FrontendPaymentRequest request) {
        ...
        try {
            ...
            log.error("결제 요청 실패, 예약 취소 진행 (paymentId={}): {}", paymentId, e.getMessage());
            self.cancelReservation(paymentId);
            throw e;
        }
    ...
    public void cancelPayment(String paymentId, String email) {
        ...
        if (reservation.getStatus() == ReservationStatus.PAID) {
            try {
                ...
                log.error("외부 결제 취소 실패 (paymentId={}): {}", paymentId, e.getMessage());
            }
        }
    ...
    private void validateSeatNotReserved(Long scheduleId, List<Long> seatIds) {
        ...
        if (isAlreadyOccupied) {
            log.warn("좌석 선점 충돌 (scheduleId={}, seatIds={})", scheduleId, seatIds);
