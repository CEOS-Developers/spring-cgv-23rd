...
public record MovieLikeResponse(
        ...
        @Schema(description = "영화 ID", example = "5")
        Long movieId,

        @Schema(description = "찜 여부 (true: 찜 완료, false: 찜 취소)", example = "true")
        boolean isLiked
) {
    public static MovieLikeResponse of(Long userId, Long movieId, boolean isLiked) {
        return new MovieLikeResponse(userId, movieId, isLiked);
    }
}