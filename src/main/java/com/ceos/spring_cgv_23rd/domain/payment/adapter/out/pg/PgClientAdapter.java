...
public class PgClientAdapter implements PgClientPort {
	...
	public PgPaymentResult pay(Payment payment) {
		...
		try {
			log.info("Calling PG pay API. amount={}", payment.getAmount());
			...
			if (response == null || response.payload() == null) {
				log.warn("PG response payload is null");
				return PgPaymentResult.fail("No payload in response");
			}

			boolean paid = "PAID".equalsIgnoreCase(response.payload().paymentStatus());
			if (!paid) {
				log.warn("PG payment not PAID. paymentStatus={}", response.payload().paymentStatus());
				return PgPaymentResult.fail("Unexpected paymentStatus: " + response.payload().paymentStatus());
			}

			log.info("PG pay succeeded");
			return PgPaymentResult.success(response.payload().pgProvider(), response.payload().paidAt());

		} catch (RestClientResponseException e) {
			PgErrorResponse error = parseError(e.getResponseBodyAsString());
			log.warn("PG error response. httpStatus={}, pgStatus={}, message={}",
				e.getStatusCode(),
	...
	public void cancel(String paymentId) {
		try {
			...
			log.warn("PG cancel failed. httpStatus={}, body={}",
