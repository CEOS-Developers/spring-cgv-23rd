...
class ReservationConcurrencyTest {
    ...
    void createReservation_allowsOnlyOneReservationForSameSeat() throws Exception {
        ...
        Future<Void> firstRequest = executorService.submit(
                reserveSeatTask(user1.getId(), screening.getId(), seatTemplate.getId(), readyLatch, startLatch,
                        successCount, conflictCount)
        );
        Future<Void> secondRequest = executorService.submit(
                reserveSeatTask(user2.getId(), screening.getId(), seatTemplate.getId(), readyLatch, startLatch,
                        successCount, conflictCount)
        );
